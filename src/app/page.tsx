  ...
  return (
    <Shell>
      <div className="space-y-10">
        {/* Royal Header */}
        <div className="flex justify-between items-center bg-card p-8 rounded-3xl border-2 border-secondary/20 shadow-xl shadow-secondary/5">
          <div>
            <h2 className="text-4xl font-black text-white tracking-tight">Sala de Comando</h2>
            <p className="text-white/60 font-medium mt-2">Visão consolidada do tesouro patrimonial do Reino.</p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {stats.map((stat) => (
            <div key={stat.label} className="bg-card border-2 border-secondary/10 p-8 rounded-3xl shadow-lg hover:border-secondary transition-all group relative overflow-hidden">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          <div className="bg-card border-2 border-secondary/10 rounded-3xl p-8 shadow-lg">
            <div className="flex items-center justify-between mb-8">
              <h3 className="text-xl font-black text-white flex items-center gap-2">
                <Zap className="w-5 h-5 text-secondary" /> Valorização do Acervo
              </h3>
              <select className="bg-secondary/10 border-none rounded-xl px-4 py-2 text-xs font-bold text-white">
        ...
        {/* Recent Transactions */}
        <div className="bg-card border-2 border-secondary/10 rounded-3xl overflow-hidden shadow-lg">
          <div className="p-8 border-b border-secondary/10 flex justify-between items-center">
            <h3 className="text-xl font-black text-white">Atos Recentes de Movimentação</h3>
            <button className="px-6 py-2 bg-secondary/10 text-secondary rounded-xl text-xs font-black uppercase hover:bg-secondary hover:text-background transition-all">
              Ver Histórico Real
            </button>
          </div>
            ...
            {[1, 2, 3].map((i) => (
              <div key={i} className="p-6 flex items-center justify-between hover:bg-secondary/5 transition-colors">
                <div className="flex items-center gap-6">
                  ...
                  <div>
                    <p className="text-sm font-black text-white">Estação de Trabalho Real #00{i}</p>
                    <p className="text-xs text-white/60 font-medium">Movimentado para: Setor de Governança</p>
                  </div>
                </div>
                <div className="text-right">
                  <p className="text-xs font-bold text-white/40 uppercase">Há {i * 5} min</p>
